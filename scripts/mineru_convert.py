...
class MinerUConverter:
    ...
    def _rename_output_files(self, output_dir: Union[str, Path], base_name: str) -> str:
        """Rename extracted files to match input filename.

        Renames:
        - full.md → {base_name}.md
        - content_list_v2.json → {base_name}_content_list.json
        - layout.json → {base_name}_layout.json
        - {uuid}_content_list.json → {base_name}_content_list_raw.json
        - {uuid}_model.json → {base_name}_model.json
        - {uuid}_origin.pdf → {base_name}_origin.pdf

        Args:
            output_dir: Directory containing extracted files
            base_name: Base name to use for renamed files

        Returns:
            Path to the main .md file after renaming
        """
        output_dir_path = Path(output_dir)

        # Mapping of old names to new names
        rename_map = {
            "full.md": f"{base_name}.md",
            "content_list_v2.json": f"{base_name}_content_list.json",
            "layout.json": f"{base_name}_layout.json",
        }

        # Rename standard files
        for old_name, new_name in rename_map.items():
            old_path = output_dir_path / old_name
            if old_path.exists():
                old_path.rename(output_dir_path / new_name)

        # Rename UUID-prefixed files
        for file_path in output_dir_path.iterdir():
            if file_path.is_file():
                # Match pattern: {uuid}_content_list.json
                if file_path.name.endswith("_content_list.json") and file_path.name != f"{base_name}_content_list.json":
                    file_path.rename(output_dir_path / f"{base_name}_content_list_raw.json")
                # Match pattern: {uuid}_model.json
                elif file_path.name.endswith("_model.json"):
                    file_path.rename(output_dir_path / f"{base_name}_model.json")
                # Match pattern: {uuid}_origin.pdf
                elif file_path.name.endswith("_origin.pdf"):
                    file_path.rename(output_dir_path / f"{base_name}_origin.pdf")

        # Return path to main md file
        md_path = output_dir_path / f"{base_name}.md"
        if md_path.exists():
            return str(md_path)
    ...
    def convert(
        ...
        # Default structure: <parent>/transcript/<stem>/hybrid_ocr/
        if output_dir:
            # User specified output directory - use it as-is
            output_dir_path = Path(output_dir)
        elif input_path:
            # Create transcript/<stem>/hybrid_ocr/ structure
            input_path_obj = Path(input_path)
            stem_name = input_path_obj.stem  # filename without extension
            output_dir_path = input_path_obj.parent / "transcript" / stem_name / "hybrid_ocr"
        else:
            # URL input without output_dir - use current directory
            output_dir_path = Path.cwd() / "transcript" / "url_input" / "hybrid_ocr"
        ...
        if input_path:
            ...
            # Rename output files to match input filename
            if input_path:
                base_name = Path(input_path).stem
                output_file = self._rename_output_files(output_dir_path, base_name)

...
def _deduplicate_overlap(old_text: str, new_text: str) -> str:
    ...
    # Look back up to 200 characters for a match (increased for robustness)
...
def call_llm(model_name: str, system: str, user: str, temperature: float = 0.0,
    ...
    def timer_thread(part_num):
        ...
        while not first_token_received.is_set():
            ...
            label = f"Waiting for model response (Part {part_num})"
    ...
    try:
        while True:
            ...
            try:
                ...
                # Check for stagnation: if the model returned nothing or just a few chars, retry or break
                if continuation_count > 1 and len(part_text.strip()) < 10:
                    print(f"\n[Warning] Part {continuation_count} was empty or too short. Retrying...", flush=True)
                    # We don't increment continuation_count here, just retry the same part
                    # but to avoid infinite loop, we add a mini-counter or just break after 3 failures
                    if not hasattr(call_llm, "_retry_count"): call_llm._retry_count = 0
                    call_llm._retry_count += 1
                    if call_llm._retry_count > 3:
                        print("  [Error] Model failed to continue multiple times. Ending generation.")
                        break
                    continue
                
                call_llm._retry_count = 0 # Reset on success
                ...
                if continuation_count < 20: # Increased limit to 20
                    # 1. API Limit
                    ...
                    # 3. Micro-check for mid-command or mid-math
                ...
                if is_truncated:
                    continuation_count += 1
                    last_chars = part_text[-50:] # Provide context anchor
                    math_hint = ""
                    ...
                    # Improved prompt with contextual anchor
                    prompt_msg = (
                        f"Your previous response was cut off right after the characters: '{last_chars}'.\n"
                        f"Please continue EXACTLY from that point. {math_hint}\n"
                        "DO NOT repeat any characters you have already printed. Output ONLY the remaining content."
                    )
                    messages.append({"role": "user", "content": prompt_msg})
                else:
                    break
            except Exception as e:
                print(f"\n[Error] API call failed during Part {continuation_count}: {e}. Retrying in 2s...")
                time.sleep(2)
                continue
    finally:
        first_token_received.set()
